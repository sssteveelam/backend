...
type ThresholdLineInput = {
  quantityBase: number;
  baselineQuantityBase?: number;
};

type EvaluateThresholdInput = {
  actorUserId: string;
  documentType: 'receipt' | 'movement';
  documentValue: number;
  lines: ThresholdLineInput[];
};

type EvaluateOpenSealInput = {
  actorUserId: string;
...
export class ThresholdService {
  ...
  async evaluate(input: EvaluateThresholdInput) {
    const now = new Date();
    ...
    // Threshold evaluation is executed after document submit, so current document
    // is already included in submitted totals and must not be counted twice.
    ...
    const lineEvaluations = input.lines.map((line) => {
      ...
      const percentDelta =
        baseline && baseline > 0 ? Math.abs(line.quantityBase - baseline) / baseline : 0;
  ...
  async evaluateOpenSeal(input: EvaluateOpenSealInput) {
